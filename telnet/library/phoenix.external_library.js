...
if(do_yesno_lightbar && (!(bbs.sys_status&SS_USERON) || (user.security.exemptions&UFLAG_R))) {
	bbs.replace_text(559,"@NOPAUSE@@EXEC:external/phoenix.yesnobar@@NOPAUSE@");	//defaults on yes...
	bbs.replace_text(562,"@NOPAUSE@@EXEC:external/phoenix.noyesbar@@NOPAUSE@");	//defaults on no... 

//if disabled...
}else{
	bbs.replace_text(559,"@NOPAUSE@@EXEC:external/phoenix.yesno@@NOPAUSE@"); //defaults on yes...
	bbs.replace_text(562,"@NOPAUSE@@EXEC:external/phoenix.noyes@@NOPAUSE@"); //defaults on no... 
}
