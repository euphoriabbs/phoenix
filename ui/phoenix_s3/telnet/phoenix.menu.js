...
//add flags required for user logon experiance [if configured]...
...
console.print('\1nbÞÞÝh±Þnþ \1n\1creading messages...');
...
bbs.replace_text(19,"\1nbÞÞÝh±Þnþ phkosting on \1n%s %s"); 
...
//message scanner...		
bbs.replace_text(67,'[24;1H\1nbÞÞÝh±Þnþ -c%-15.15s yh%-40.40s\1,\1,'); //display the subz scanned...
bbs.replace_text(116,''); //message scan...	
bbs.replace_text(117,''); //sub totals...

//post data information for message menu...	
bbs.replace_text(33,"\r\n\1nbÞÞÝh±Þnþ shkaving..."); //saving statement...
bbs.replace_text(34,"\r\n\1nbÞÞÝh±Þnþ shkaved \1n%lun characters (n%un lines)...n"); //saved statement...
bbs.replace_text(35,"\r\n\1nbÞÞÝh±Þnþ whkriting index..."); //writing index statement...
bbs.replace_text(37,"\r\n\1nbÞÞÝh±Þnþ phkosted on \1n%s %s\r\n"); //posted on statement...
...
bbs.replace_text(661,"@NOPAUSE@@CLS@@TYPE:../../ui/phoenix_s3/artwork/art.phoenix.file_lister.ans@@NOPAUSE@"); 
...
var strHeaderFile = "../../ui/phoenix_s3/artwork/art.phoenix.menu_oneliner.ans"; //ansi header...
...
bbs.menu.ansi_gallery = function() {
	...
	var string = directory('../../ui/phoenix_s3/artwork/gallery/*.ans');
...
console.putmsg("@TYPE:../../ui/phoenix_s3/artwork/" + fname + ".ans@"); 
...
bbs.mods.vanguard.ansislow = function(fname) {
	file_base = "../../ui/phoenix_s3/artwork/" + fname;
	if (file_exists(file_base))
		f = new File(file_base);
		...
		console.print("file doesn't exist: ../../ui/phoenix_s3/artwork/" + fname + ".???\r\n");
		return;
	}
	if(!f.open("r")) {
		console.print("error opening file: " + f.name + "\r\n");
